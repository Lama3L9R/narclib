...
namespace narchook::archash {
    ...
    EncryptedAPI constexpr size_t challenge_size() {
    ...
    EncryptedAPI uint64_t get_ts_now() {
        ...
        return (tmp.QuadPart - 0x019DB1DED53E8000) / 100000;
#endif
    }
    ...
    EncryptedAPI void sponge(uint96_t* raw, uint96_t** result) {
        ...
        if (*result == nullptr) {
            (*result) = (uint96_t*) mem::alloc(len);
        }

        memcpy(*result, buff, len);
    }

    EncryptedAPI void hash(const char* raw, size_t len, char** result) {
        uint96_t buff[6] = { };

        size_t n = 0;
        while (n + 6 <= len) {
            buff[0].low ^= *((uint64_t*) (raw + n)) & 0xFFFFFFFFFFFF;
        ...
        if (*result == nullptr) {
            *result = (char*) mem::alloc(4 * sizeof(uint96_t));
        }

        uint96_t* datahash;
        sponge(buff, &datahash);
        memcpy(*result, datahash, 2 * sizeof(uint96_t));
        free(datahash);
        datahash = nullptr;

        sponge(buff, &datahash);
        memcpy(*result + 2 * sizeof(uint96_t), datahash, 2 * sizeof(uint96_t));
        free(datahash);
        datahash = nullptr;
    }

    EncryptedAPI void generate_challenge_pt2(char* body, arc_salt_t salt, uint64_t ts, char** result) {
        size_t body_len = strlen(body);

        size_t buffer_len = sizeof(uint64_t) + body_len + sizeof(arc_salt_t);
        char* buff = (char*) mem::alloc(buffer_len);

        *((uint64_t*) buff) = ts;
        memcpy(buff + sizeof(uint64_t), body, body_len);
        memcpy(buff + sizeof(uint64_t) + body_len, salt, sizeof(arc_salt_t));

        hash(buff, buffer_len, result);

        free(buff);
    }

    EncryptedAPI void generate_challenge_pt1(char* body, char* path, arc_salt_t salt, uint64_t ts, char** result) {
            size_t body_len = strlen(body);
